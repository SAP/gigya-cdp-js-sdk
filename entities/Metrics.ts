...
function getMetrics<T extends string>(metricsRes: MetricResponse<T>, aggTags: T[], filterTags: FilterTag<T>[] = []): any {
  ...
  if (aggTags.length)
    return {
      get count() {
        return Object.values(this.components ?? {}).reduce((res: number, c: { count: number }) => res + c.count ?? 0, 0);
      },
